...
const SlidersTable = () => {
  ...
  const confirmDeleteSlider = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/Slider/${sliderToDelete.id}`,
        {
          method: "DELETE",
        }
      );
      if (response.ok) {
        const updatedMaster = masterSliders.filter(
          (p) => p.id !== sliderToDelete.id
        );
  ...
  const handleEditSlider = useCallback((slider) => {
    setSelectedSlider(slider);
    setIsEditDrawerOpen(true);
  }, []);
  const handleDeleteSlider = useCallback((slider) => {
  ...
  const totalPages = useMemo(
    () => Math.ceil(filteredSliders.length / slidersPerPage),
            ...
            {currentSliders.map((slider) => (
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-left">
                  <span className="truncate max-w-xs" title={slider.title}>
                    {slider.title || "Chưa có tên"}
                  </span>
                </td>
                    ...
                    <img
                      src={
                        slider.imageUrl.startsWith("http")
                          ? slider.imageUrl
                          : `${process.env.REACT_APP_API_BASE_URL}/${slider.imageUrl}`
                      }
      ...
      >
        ...
        >
          <DialogContentText sx={{ color: "rgb(209,213,219)" }}>
            Bạn có chắc chắn muốn xóa Slider "{sliderToDelete?.title}" không?
